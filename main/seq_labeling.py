...
def generate_sequence_output(encoder_outputs,
    ...
    with tf.name_scope(name, "model_with_buckets", tf.unstack(all_inputs)):
        with tf.variable_scope("decoder_sequence_output", reuse=None):
            ...
            if per_example_loss is None:
                ...
                assert len(logits) == len(tf.unstack(targets)), "logits与targets长度不相等"
                bucket_target = [tf.reshape(tf.to_int64(x), [-1]) for x in tf.unstack(targets)]
                crossent = sequence_loss(logits, bucket_target, tf.unstack(weights), softmax_loss_function=softmax_loss_function)
